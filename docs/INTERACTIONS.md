...
The complete range maps to interior pixel centres. `0` remains half a pixel inside the near edge and `1` remains half a pixel inside the far edge, so both accepted endpoints stay within the viewport or target box. This keeps pointer recipes usable across viewport sizes and responsive layouts.
...
Locator clicks use one deadline across visibility resolution, scrolling, bounding-box resolution, and the click action. Target-relative drag endpoints retain separate declared resolution budgets because both targets may require independent scrolling and visibility waits.
